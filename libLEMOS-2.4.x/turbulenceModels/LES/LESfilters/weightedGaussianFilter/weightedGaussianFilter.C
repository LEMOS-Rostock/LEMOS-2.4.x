...
#include "weightedGaussianFilter.H"
...
defineTypeNameAndDebug(weightedGaussianFilter, 0);
addToRunTimeSelectionTable(LESfilter, weightedGaussianFilter, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// from components
weightedGaussianFilter::weightedGaussianFilter
(
    const fvMesh& mesh,
    const scalar gamma
)
...
weightedGaussianFilter::weightedGaussianFilter(const fvMesh& mesh, const dictionary& dict)
:
    LESfilter(mesh),
    addressing_(centredCPCCellToCellExtStencilObject::New(mesh)),
...
void weightedGaussianFilter::read(const dictionary& dict)
{
    stencilWeights();
}

void weightedGaussianFilter::stencilWeights()
{
	...
	if(nElements > 0)
	{
		...
		stencilWeight.append(G);
            ...
            {
	        stencilWeight[stencilWeightI] /= sumG;
            }
	}
    }
}

void weightedGaussianFilter::updateStencil()
{
...
Foam::tmp<Foam::volVectorField> Foam::weightedGaussianFilter::operator()
(
...
Foam::tmp<Foam::volScalarField> Foam::weightedGaussianFilter::operator()
(
...
Foam::tmp<Foam::volSymmTensorField> Foam::weightedGaussianFilter::operator()
(
...
Foam::tmp<Foam::volTensorField> Foam::weightedGaussianFilter::operator()
(
