...
void Foam::cellToCellExtStencil::merge
(
    const label global0,
    const label global1,
...
)
{
    ...
    {
        FatalErrorIn("cellToCellExtStencil::merge(..)")
...
void Foam::cellToCellExtStencil::merge
(
    const labelPairList& trafoElements,
    labelPairList& cCells
)
{
    Map<labelPair> set;
    ...
    forAll(cCells, i)
    {
            set.insert(cCells[i].first(), cCells[i]);
    }

    forAll(trafoElements, i)
    {
            set.insert(trafoElements[i].first(), trafoElements[i]);
    }

    cCells.setSize(set.size());
    label n = 0;

    forAllConstIter(Map<labelPair>, set, iter)
    {
        cCells[n++] = iter();
    }
    set.clearStorage();
...
void Foam::cellToCellExtStencil::validBoundaryFaces(boolList& isValidBFace) const
{
    ...
    forAll(patches, patchI)
    {
        ...
        if (isA<emptyPolyPatch>(pp) || isA<coupledPolyPatch>(pp))
...
Foam::cellToCellExtStencil::coupledFacesPatch(const polyPatch& pp) const
{
    label nCoupled = 0;
...
Foam::autoPtr<Foam::indirectPrimitivePatch>
Foam::cellToCellExtStencil::allCoupledFacesPatch() const
{
    const polyBoundaryMesh& patches = mesh().boundaryMesh();
...
void Foam::cellToCellExtStencil::insertFaceCells
(
...
Foam::labelList Foam::cellToCellExtStencil::calcFaceCells
(
    const boolList& isValidBFace,
    const labelList& faceLabels,
    labelHashSet& globals
...
Foam::cellToCellExtStencil::cellToCellExtStencil(const polyMesh& mesh)
:
    mesh_(mesh),
